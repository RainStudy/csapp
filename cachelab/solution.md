...
要我们写一个 cache simulator，读取 `valgrind` 提供的内存读取/写入信息，输出 hit 和 miss 的次数。

lab 文件夹中已经提供了一个 cache simulator 实现的二进制文件 `./csim-ref`
...
其实就是利用分块和展开减少miss次数，代码太长就不贴了。

![image-20230518154037015](https://persecution-1301196908.cos.ap-chongqing.myqcloud.com/image_bedimage-20230518154037015.png)

唯一丢分的地方挺怪的 我用 csim 自己测就跟标准答案一致，但是用 ./test-csim 就是有问题
